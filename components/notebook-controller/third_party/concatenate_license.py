...
def main():
    with (
        open(args.license_info_file, "r") as license_info_file,
        open(args.output_file, "w") as output_file,
    ):
        repo_failed = []
        for line in license_info_file:
            line = line.strip()
            [repo, license_link, license_name, license_download_link] = line.split(",")
            try:
                print(
                    "Repo {} has license download link {}".format(
                ...
                print(
                    "{}  {}  {}".format(repo, license_name, license_link),
                    file=output_file,
                )
                print(
                    "--------------------------------------------------------",
        ...
        print(
            "Failed to download license file for {} repos.".format(len(repo_failed)),
            file=sys.stderr,
        )
